...
public class SrsStreamerNormal : MonoBehaviour
{
    private const string DefaultPath = "/rtc/v1/publish/";

    public string DefaultSchema = "http:";

    public string url = "webrtc://localhost/live/livestream";
    ...
    void Start()
    {
        ...
        pc.OnIceCandidate = candidate => { Debug.Log($"WebRTC: OnIceCandidate {candidate.ToString()}"); };
        pc.OnIceConnectionChange = state => { Debug.Log($"WebRTC: OnIceConnectionChange {state.ToString()}"); };
        pc.OnTrack = e => { Debug.Log($"WebRTC: OnTrack {e.Track.Kind} id={e.Track.Id}"); };
        ...
        IEnumerator GrabCamera()
        {
            ...
            Debug.Log(
                $"WebRTC: Grab camera stream={videoStream.Id}, size={sourceCamera.targetTexture.width}x{sourceCamera.targetTexture.height}");
        ...
        IEnumerator ExchangeSDP(string url, string offer)
        {
            ...
            var task = System.Threading.Tasks.Task<string>.Run(async () =>
            {
                ...
                SrsUtil.ConfObject confObject = SrsUtil.PrepareUrl(url, DefaultPath, DefaultSchema);

                var sdpData = new SrsUtil.SdpData
                {
                    api = confObject.apiUrl,
                    tid = confObject.tid,
                    streamurl = confObject.streamUrl,
                    clientip = null,
                    sdp = offer
                };

                var json = JsonConvert.SerializeObject(sdpData);
                var content = new System.Net.Http.StringContent(json);
                content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/json");

                var client = new System.Net.Http.HttpClient();
                var res = await client.PostAsync(confObject.apiUrl, content);
                res.EnsureSuccessStatusCode();

                string data = await res.Content.ReadAsStringAsync();
                var jObject = JObject.Parse(data);
                Debug.Log($"WebRTC: Exchange SDP ok, answer is {data}");
                return (string) jObject["sdp"];
            });
